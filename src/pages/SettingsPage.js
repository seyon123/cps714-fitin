...
function SettingsPage({ authComponent }) {
  ...
  const { currentUser, updateUserInfo, getUser, updateUserPassword, logOut } =
    useAuth();
  ...
  async function handleProfileSection(e) {
    ...
    const storageRef = ref(
      storage,
      `profiles/${Math.floor(Math.random() * (9999 - 1000)) + 1000}-${fileName}`
    );
  ...
  return (
    <LoadingSpinner isLoading={isLoading}>
      <div className="settingsPage px-5 mw- 80">
        ...
        <div className="profileSection my-3 p-3">
          <Container fluid>
