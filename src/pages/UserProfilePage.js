...
function UserProfilePage() {
	...
	async function unfollowUser() {
		await deleteDoc(doc(db, `users/${currentUser.uid}/following`, user.id), {});
	...
	return (
		<LoadingSpinner isLoading={isLoading}>
			<Container className="mainPage" style={{ color: "white" }}>
								...
								<Row>
										...
										<h2>
											<Row className="noMargin fs-2 fw-bold">
												...
												<img
													...
													style={{ marginLeft: "-25px", marginTop: "-7px", height: "60px", width: "auto", mixBlendMode: "lighten", filter: "hue-rotate(40deg) " }}
													src="/verification.gif"
												/>
											</Row>
										</h2>
										<p className="text-white-50">
											<Row className="noMargin">{user?.bio}</Row>
										</p>
										<p>
											<Row className="noMargin">
												<a className="p-0" href={user?.website} target="_blank" rel="noreferrer">
													<b>{user?.website}</b>
												</a>
											</Row>
										</p>
									...
									<Col className="my-4 text-center">
										...
										<Row className="d-flex align-items-center justify-content-center">
											{currentUser?.uid !== userid ? (
												isFollowing ? (
													<Button className="w-50" onClick={() => unfollowUser()}>
														Unfollow
													</Button>
												) : (
													<Button className="w-50" onClick={() => followUser()}>
														Follow
													</Button>
												)
											) : (
												<Button as={Link} className="w-50" to="/settings">
													Settings
												</Button>
											)}
										</Row>
										...
										<Row className="d-flex align-items-center justify-content-between">
											<Col className="">
												<Button
													onClick={() => setChangeFollowersShow(true)}
													variant="link"
													to="/"
													style={{
														color: "inherit",
											...
											<Col className="">
												<div>
													<Button
														onClick={() => setChangeFollowingShow(true)}
														variant="link"
														to="/"
														style={{
															color: "inherit",
							...
							<Col>
								{posts?.length > 0 ? (
									posts
										...
										.map(({ id, userRef, tags, timestamp, image, description, likes }) => (
				...
				<FollowingListModal show={changeFollowingShow} onHide={() => setChangeFollowingShow(false)} setModalShow={setChangeFollowingShow} userOfPage={user} />
				{/* Change Routine Modal */}
				<FollowersListModal show={changeFollowersShow} onHide={() => setChangeFollowersShow(false)} setModalShow={setChangeFollowersShow} userOfPage={user} />
			</Container>
		</LoadingSpinner>
	);
}
